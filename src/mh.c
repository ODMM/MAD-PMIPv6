...
int mh_opts_dup_ok[] = {
	...
	0, /* Access technology Type option */
	0, /* MN link layer identifier option */
	0, /* Link Local Address option */
	0, /* Timestamp option */
	0, /* Serving MAR option */
	1, /* Previous MAR option */
	0, /* DLIF Link Layer Address option*/
};
...
static void *mh_listen(void *arg)
{
	...
	while (1) {
		...
		//dbg("Length of received message %d\n",len);
		//dbg("sizeof(struct ip6_mh) = %d\n",sizeof(struct ip6_mh));
		if (len == -EBADF)
			break;
...
// 	/* CJBC */
// 	ba->ip6mhba_flags |= IP6_MH_BU_PROXY;
