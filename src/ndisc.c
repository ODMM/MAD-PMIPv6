		     ...
		     uint8_t *hwa, int hwalen)
{
	...
	if (hwa)/*{
			for (int j=0; j<hwalen; j++) {
					hwa[0]=mn_llid->s6_addr[j+10];
					printf("%02x:", mnhw[0]);
				}
				printf("\n");*/
...
int neigh_add_dlif(int ifindex, struct in6_addr *dst, uint8_t *hwa)
{
	return neigh_mod(NLM_F_CREATE | NLM_F_EXCL,
			 RTM_NEWNEIGH, ifindex, NUD_REACHABLE, 0, dst, hwa, 6);
}
