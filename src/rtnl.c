...
int dlif_modify(int cmd, unsigned int flags, int link, uint8_t *mac_address, char *name)
{
	int len;
	char *type = "macvlan";
	...
	if (mac_address){
		addattr_l(&req.n, sizeof(req), IFLA_ADDRESS, mac_address, 6);
	}
	
	if (link) 
		addattr_l(&req.n, sizeof(req), IFLA_LINK, &link, 4);
	else {
		dbg("Cannot find device %d\n",link);
		return -1;
	}
	...
	if (name) {
		len = strlen(name) + 1;
		if (len == 1) {
			dbg("\"\" is not a valid device identifier\n");
			return -1;
		}
		if (len > IFNAMSIZ) {
			dbg("\"name\" too long\n");
	...
	return rtnl_route_do(&req.n, NULL);
}

int dlif_add(int iface, char *dlif_name, uint8_t *mac_address)
{
	int ret;
	ret = dlif_modify(RTM_NEWLINK,NLM_F_CREATE|NLM_F_EXCL, iface, mac_address, dlif_name);
	if (ret >= 0) {
		int iif = if_nametoindex(dlif_name);
		ret = dlif_modify(RTM_NEWLINK,0, iif, NULL, dlif_name);
		if (ret >= 0)
			return iif;
	}
	return -1;
...
int dlif_del(int iface)
{
	char dlif_name[IFNAMSIZ];
	char *name;
	if (name = if_indextoname(iface, dlif_name)) {
		dlif_modify(RTM_DELLINK,0, iface, NULL, dlif_name);
	}
	return 0;
}